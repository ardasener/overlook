...
The terminal SHALL use an opaque WebGL surface when no wallpaper background is configured and SHALL allow transparency when a wallpaper background is configured. The background settings SHALL explain that transparent terminal rendering can make small text appear thinner or softer and that clearing the background provides the sharpest text rendering.
